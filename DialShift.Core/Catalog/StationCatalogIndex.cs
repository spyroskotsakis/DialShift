...
public sealed class StationCatalogIndex
{
    ...
    /// <summary>One entry's precomputed search keys: the folded Name, NameLocal and City (§3.3 <c>Fold</c>) and the
    /// FrequencyFm digits ("" when it has none, which never matches a frequency query).</summary>
    internal readonly record struct SearchKeys(string Name, string NameLocal, string City, string FrequencyDigits);
}
