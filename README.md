...
> **Testing status of 0.5.0.** On Windows, the build, the automated checks, the app's smoke test and the new setup's silent install, upgrade, refusal and uninstall cases pass in CI on GitHub-hosted `windows-latest` machines, and one check was done by hand for 0.4.1 in a Windows 11 ARM64 virtual machine (Parallels, the x64 app under emulation): with the stations' root certificate removed from Windows, 0.4.1 started the three starter stations and a catalog station 12 times out of 12, where 0.4.0 played none, and Windows installed the missing root certificate by itself (decision D100). This version has **not yet been tested by hand on a physical Windows PC**: the tray, audible playback, launch at sign-in, sleep and wake, upgrading with `Install.ps1`, and the setup's upgrade of an `Install.ps1` install, keyboard use and Settings → Apps page (native check NC-19) are unverified on real hardware; NC-19 has so far run in part, in a Windows 11 ARM64 virtual machine (Parallels, the x64 setup under emulation, display scale 200 %), not a physical PC (a fresh install, SmartScreen, the wizard, the uninstaller). The Mac build has been tested on the developer's Apple Silicon Mac only, not on a clean Mac. Please report problems as [issues](https://github.com/spyroskotsakis/DialShift/issues), with your `dialshift.log` (see [Data](#data)). The full list of open checks is in [Open items](#open-items--release-status).
>
> **New in 0.5.0: a Windows setup,** `DialShift-Setup-win-x64.exe` ([Install on Windows](#install-on-windows); see `[0.5.0]` in [CHANGELOG.md](CHANGELOG.md)). The app itself is 0.4.1's, with one log fix (a line of `dialshift.log` could be lost when two DialShift processes wrote at once). The setup's build, content check and silent install, upgrade and uninstall cases pass in CI, which every release must pass before it is published; by hand it has so far run in part, in a Windows 11 ARM64 virtual machine (Parallels, the x64 setup under emulation, display scale 200 %), not a physical PC (NC-19).
>
> **Since 0.4.0: the station catalog search** (the Add station dialog below; see `[0.4.0]` in [CHANGELOG.md](CHANGELOG.md); 0.4.1 fixes `https://` stations on Windows). Its build, automated checks, package checks and smoke test pass in CI on `windows-latest` and `macos-latest`, and it has been tested on the developer's Apple Silicon Mac. It has not yet been used by hand on a real Windows PC, and it has had no screen-reader or real-logo check by hand on either OS.

## Download

Download DialShift from the **[Releases page](https://github.com/spyroskotsakis/DialShift/releases)**. Every release has these files (releases up to `v0.4.1` have no setup, and their checksum file covers the two zips):
...
**Releases and "latest".** The "latest" links below always point at the newest full release, now `v0.5.0` (`v0.3.0` was the first). Pre-releases, such as `v0.3.0-rc.1` and `v0.3.0-rc.2`, are listed on the [Releases page](https://github.com/spyroskotsakis/DialShift/releases) but are never the "latest" release. `v0.3.0`, `v0.4.0`, `v0.4.1` and `v0.5.0` were released before every native check had passed; see the testing status above and [Open items](#open-items--release-status).

- Latest release: <https://github.com/spyroskotsakis/DialShift/releases/latest>
- Windows setup (from `v0.5.0` on): <https://github.com/spyroskotsakis/DialShift/releases/latest/download/DialShift-Setup-win-x64.exe>
...
A specific release's files are always at `https://github.com/spyroskotsakis/DialShift/releases/download/<tag>/<file>`, for example `…/download/v0.5.0/DialShift-Setup-win-x64.exe`. What changed in each release is in [CHANGELOG.md](CHANGELOG.md).
...
- **Automation:** `DialShift-Setup-win-x64.exe /S` installs silently (no desktop shortcut, DialShift is not started) and `"%LOCALAPPDATA%\Programs\DialShift\Uninstall DialShift.exe" /S` uninstalls silently and keeps the settings; add `_?=%LOCALAPPDATA%\Programs\DialShift` to wait for the uninstaller to finish (the uninstaller file itself then stays for you to delete). `/D=<folder>`, last on the command line, in capitals and without quotes (even when the folder has spaces), installs into another folder; it is meant for automation and tests. The setup installs only into exactly the folder `/D=` names: a folder it can't use (a relative path, a drive that doesn't exist, a file in its place, a drive or network share root), anything that makes Windows read another folder (a forward slash, quotes, a wildcard or a control character in it, or another switch after it, such as `/D=C:\Apps\DialShift /S`), a `/D=` it can't read (quoted as a whole, lowercase), a command line longer than 1,023 characters or a folder path too long for DialShift's files under Windows' 259-character path limit stops it with exit code 13, and it never installs into another folder instead. A `\\?\` path is accepted only before a drive letter or `UNC\`, and only when it names the same folder without the prefix. The uninstaller likewise works only in exactly the folder `_?=` names (last on the command line, with backslashes), and only in a folder that holds a regular file `Uninstall DialShift.exe`; when Windows runs it from its temporary copy (Settings → Apps, a double-click: no `_?=`), that file must also have the copy's size and a last-write time within 2 seconds of it, so another install is refused unless its uninstaller was written within those 2 seconds; it refuses a drive or share root (`_?=C:\`), a `_?=` that is not an ordinary full path (such as `\\?\Volume{…}\…`) or another folder with 13 (`/D=` does not apply to the uninstaller). It removes the Start menu and desktop shortcuts, the launch-at-sign-in value and the Settings → Apps entry only when they point at that folder; a `_?=` that is not a full path on an existing drive or share (`_?=C:`, `_?=DialShift`) makes it exit 2 before it starts (NSIS's own "Error launching installer" when not silent), with nothing deleted. Exit codes of the setup and the uninstaller: 0 done, 1 cancelled, 10 DialShift is running, 11 another install is running, 12 not 64-bit Windows 10 or later, 13 a refused folder (the settings folder, also through an 8.3 name, a subst drive, a junction or a mount point when Windows can resolve both paths (if it can't, only the plain spelling is recognized), a drive or network share root, a folder with other files, a `/D=` folder or switch the setup can't use, a folder path too long, or a second install location while DialShift is installed elsewhere; for the uninstaller, a drive or share root or a path that isn't an ordinary full path), 14 the install failed and the previous install is unchanged, or the uninstaller could not delete a file (it names it; the entry in Settings → Apps stays, so you can uninstall again), 15 `DialShift.exe` could not be opened to check whether DialShift is running.
...
- **Going back to an older version** keeps your stations and schedule. From a setup install, uninstall DialShift in Settings → Apps first (keeping your settings), then use the older version's zip: `Install.ps1` refuses to replace a setup install, and releases before `v0.5.0` have no setup. `v0.3.0` drops the notes of stations added from the catalog the next time it saves; the apps before it also ignore slot time zones and remove them.

## Open items / release status

Both packages build, test and pass the native smoke in CI. Some checks still need things CI can't provide: Windows hardware, a clean Mac, signing credentials, a real login and a person at the screen. `v0.3.0` was released as a full release before those checks (decision D57 in [docs/decisions.md](docs/decisions.md)), and so are `v0.4.0` (D92), `v0.4.1` (the D100 fix) and `v0.5.0` (the Windows setup, D93–D99, D101), with the testing status stated in the release notes and at the top of this README; the checks stay open and are tracked for the next releases. [docs/open-items.md](docs/open-items.md) lists every one of them, with why it is blocked and how to run and record it.
...
- **Development (what the scripts produce):** the Windows build, its setup and the setup's uninstaller are unsigned; the macOS bundle is ad-hoc signed. Used for local and CI builds and, so far, for every release, including `v0.3.0` (D57), `v0.4.0`, `v0.4.1` and `v0.5.0`. Gatekeeper and SmartScreen will warn.
- **Signed release (not done yet):** Authenticode on Windows; Developer ID signing with the hardened runtime, notarization and stapling on macOS. Both need credentials and a clean-machine install test. `v0.3.0` (decision D57), `v0.4.0`, `v0.4.1` and `v0.5.0` shipped in the development tier before them.
