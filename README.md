...
|---|---|---|---|
| `DialShift-Setup-win-x64.exe` | Windows 10/11, x64 | LibVLC (bundled) | Windows setup of the package below (per user, no administrator rights); unsigned; new since `v0.4.0`: CI is set up to build it on `windows-latest` and on macOS, check it and run its install, upgrade and uninstall silently, and the first of those runs is still pending; it has not yet been run by hand on a real Windows PC (NC-19) |
