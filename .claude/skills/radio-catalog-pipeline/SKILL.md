...
3. **Verify:**
   ...
   - The run printed `app-catalog: working=… url_excluded=… duplicates_removed=… exported=… -> data/output/app-catalog.json` and `app-catalog: languages=… unknown=0`, and exited 0. A non-zero exit is a bad `data/languages.yaml` (stopped before anything is written) or a failed app-catalog validation: every problem is printed and the previous JSON and XLSX are left untouched.
