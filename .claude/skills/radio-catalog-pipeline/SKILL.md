...
1. **Edit the YAML source of truth only:**
   ...
   - Frequency band words: `data/frequency-bands.yaml` (`band_words`, e.g. `Shortwave`) lists the only words a station's `freq` may be instead of an FM value with a `.` (64–108) or a kHz integer (150–30,000, the top of shortwave). Keep `freq` a number; add a word here only for a real band (short label, at most 16 characters).
