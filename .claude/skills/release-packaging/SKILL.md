...
- `Info.plist`: `CFBundleIdentifier=com.tsiger.dialshift`, `CFBundleDisplayName`, `CFBundleIconFile=DialShift.icns`, `LSUIElement=true` (menu-bar app, no Dock icon), `NSHighResolutionCapable`, `LSMinimumSystemVersion=14.0` (decision D22: upstream precedent; the binaries load on 12.0, but the AVPlayer format corpus is verified on macOS 26.5 only, so older versions are not claimed), version from the csproj `<Version>`.
...
- The csproj sets `VlcWindowsX86Enabled`/`VlcWindowsArm64Enabled=false`, so only `libvlc/win-x64` ships.
