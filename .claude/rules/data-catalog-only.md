...
- City spellings live ONLY in each country YAML's `city_aliases` (normalized keys, one city per group of spellings, no chains; never a region to its capital or a curated entry's city to another). The app catalog formats radio-browser tag notes (`Tags: a, b`, each tag cut of non-letter, non-digit runs at its ends) in the export only; the CSVs keep the raw `tags:` note.
- Frequency band words live ONLY in `data/frequency-bands.yaml`: a station's frequency is empty, an FM value with a `.` (64–108), a kHz integer (≥ 150) or one of those words, or the app-catalog validation fails.
