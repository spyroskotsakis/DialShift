...
You own `DialShift.Tests`: a framework-free console runner (`TestHarness.cs`, with the suites registered in `Program.cs`). `Check(name, condition)` prints `PASS: name` or throws, which stops that suite; `Skip(name, reason)` prints `SKIP:` and is counted, so nothing is skipped silently; any failure gives a non-zero exit. Run with `dotnet run --project DialShift.Tests/DialShift.Tests.csproj` (`-- --filter <text>` runs matching suites).
