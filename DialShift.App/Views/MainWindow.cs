...
public sealed class MainWindow : Window
{
    ...
    public MainWindow(App app)
    {
        ...
        shell.RowDefinitions = new RowDefinitions("Auto,Auto,Auto,*,Auto");
        Content = shell;
        ...
        var playerGrid = new Grid { ColumnDefinitions = new ColumnDefinitions("*,180") };
        ...
        volume.PointerReleased += (_, _) => app.Save(); volume.KeyUp += (_, _) => app.Save();
        ...
        dial.Children.Add(new Ellipse { Stroke = Brush("#3D5141"), StrokeThickness = 1 });
        dial.Children.Add(new Ellipse { Stroke = Brush("#C2F278"), StrokeThickness = 3, Margin = new Thickness(15) });
        ...
        var tabs = new StackPanel { Orientation = Orientation.Horizontal, Margin = new Thickness(0, 0, 0, 20), Spacing = 8 };
        ...
        PropertyChanged += (_, e) => { if (e.Property == WindowStateProperty && WindowState == WindowState.Minimized) Hide(); };
    ...
    private void ShowStations()
    {
        ...
        {
            var row = new Grid { ColumnDefinitions = new ColumnDefinitions("60,*,Auto") };
            ...
            var actions = new StackPanel { Orientation = Orientation.Horizontal, VerticalAlignment = VerticalAlignment.Center, Spacing = 8 };
    ...
    {
        var editor = new StationDialog(app, station);
        if (await editor.ShowDialog<bool>(this)) app.Refresh();
    }
    ...
    private void ShowSchedule()
    {
        ...
        enabled.IsCheckedChanged += (_, _) => { app.Settings.ScheduleEnabled = enabled.IsChecked == true; app.Radio.RefreshSchedule(); app.Refresh(); };
        tools.Children.Add(enabled); page.Children.Add(tools);
        var days = new Grid { ColumnDefinitions = new ColumnDefinitions("*,*,*,*,*,*,*"), Margin = new Thickness(0, 0, 0, 15) };
        for (var i = 0; i < Week.Length; i++)
        {
            var index = i; var day = Button(Week[i].ToString()[..3], () => { selectedDay = index; RefreshPage(); }, selectedDay == i);
            day.Margin = new Thickness(i == 0 ? 0 : 4, 0, i == Week.Length - 1 ? 0 : 4, 0);
            Grid.SetColumn(day, i); days.Children.Add(day);
        }
        ...
        page.Children.Add(Text("Times follow your local time zone. Pause or pick a station manually until the next slot. After sleep, DialShift catches up with the current slot.", 12, false, "#81989A", new Thickness(0, 8, 0, 0)));
    }

    private async void EditSlot(ScheduleEntry? entry)
    {
        var editor = new ScheduleDialog(app, entry, Week[selectedDay]);
        if (await editor.ShowDialog<bool>(this)) { app.Radio.RefreshSchedule(); app.Refresh(); }
    }
    ...
    private void ShowSettings()
    {
        ...
        var login = new CheckBox { Content = "Launch DialShift in the tray when I sign in", IsChecked = app.Settings.LaunchAtLogin };
        login.IsCheckedChanged += async (_, _) =>
        {
            try { app.SetStartup(login.IsChecked == true); }
            catch (Exception ex) { login.IsChecked = app.Settings.LaunchAtLogin; await Message.Show(this, "Couldn't update startup", ex.Message); }
        };
        ...
        hidden.IsCheckedChanged += (_, _) => { app.Settings.StartInTray = hidden.IsChecked == true; app.Save(); }; startup.Children.Add(hidden);
