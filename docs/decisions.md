...
| D80 | 2026-09-25 | Catalog provider and logo loader: implementation amendments. One-pass parse from memory with a BOM strip and at most 10,000 station objects built; an unnormalizable override path is Unavailable; exact log wording; one shared download per logo URL, abandoned and not cached when every caller cancels, with a `User-Agent`; the smoke catalog check fails whenever `DIALSHIFT_CATALOG_PATH` is set; `verify-mac-app.sh` needs `/usr/bin/python3` | brief 3 §5.2, §7, §9; D60, D66, D69, D74 |
...
- **Decision:**
  1. **One-pass parse from memory, with an explicit BOM strip (§4.2 steps 2–3).** The provider reads the whole size-checked file into one byte array and parses it with one `JsonSerializer.Deserialize` call; the contract had said `DeserializeAsync` over the stream. A leading UTF-8 byte order mark is skipped explicitly, because parsing bytes, unlike a stream, does not skip it. The file is opened read-only, and a path that cannot seek (a device or a pipe, with no length to check) is Unavailable with `the path is not a regular file.`. The lane did not list that last check among its deviations, but it is in the code and it closes the gap in the size guard.
     *Rationale:* the file is at most 32 MiB (`MaxFileBytes`, already checked), so holding it costs no more than parsing it. Over a stream, the async deserializer re-scanned the buffered `stations` array for the custom converter on every buffer refill, which the lane measured at about twice the parse time on the real catalog. The pipeline writes no BOM (§2.1), but a hand-saved fixture may have one, and rejecting it would be a surprise.
  2. **At most 10,000 station objects are built, and one element converter is reused (§4.2 steps 3 and 5).** A private converter reads the `stations` array. It builds at most `MaxEntries` station DTOs and only counts the elements after them (`Utf8JsonReader.Skip`), and the count check runs before mapping.
     *Rationale:* without this, a hostile 32 MiB file of `{}` elements would allocate millions of DTOs before the "more than 10,000" rule could reject it; now allocation is bounded whatever the file holds. Such a file logs only its one `catalog.unavailable`, and no `catalog.entries_skipped`. Taking the element converter from the options once, instead of calling `JsonSerializer.Deserialize` once per element, made the whole load about 40 % faster on the real catalog (lane measurement).
  ...
  4. **Exact log wording (§4.2 log table).** `catalog.unavailable` reads `Station catalog unavailable: <reason> (<path>).`, with the reason's own final period dropped so the line has one; `<path>` is the literal `DIALSHIFT_CATALOG_PATH` when there is no path. `catalog.loaded` shows its source as the literal `[app folder]` or `[DIALSHIFT_CATALOG_PATH]`, and a missing or unparsable `generated_utc` as `(generated unknown)`. `CatalogLoadResult.Message` keeps the reason with its period. An exception from the log itself is swallowed.
     *Rationale:* one sentence per line, grep-able fixed tokens for the source, and no `(generated )` hole; tests can assert the exact shape. A failing log must not turn a degraded catalog into a fault ("never faults", §4.1).
