...
- **Update (2026-09-25, docs-lane trace, verified by the spec lane):**
  - **788 entries, not 742.** `build_stations.make_row` sets `notes=entry.get('notes', desc)`, where `desc` is the Wikipedia table's description on every row of the Wikipedia loop. The 742 `wiki+radio-browser` entries carry it, and so do 46 curated entries whose YAML entry has no `notes` of its own, all in Greece: 23 with source `curated` and 23 with `curated+radio-browser`. The spec lane joined `data/output/app-catalog.json` to `data/canonical/*.csv` on `(country, name, stream_url)` (all 8,274 matched) and classified each non-empty note: 742 + 46 are Wikipedia text; 97 are curated YAML notes; 59 are `curated: …` notes; 5,005 are radio-browser `tags: …`; 2,325 are empty. So 788 of 8,274 entries carry Wikipedia text (CC BY-SA 4.0).
