...
| D56 | 2026-09-25 | SW-N4: `Install.ps1` stages the new build beside the install and swaps it in by renames (retried about 4 s), restoring the old install on failure; refuses a source that overlaps the install folder (textual comparison); shows every message and waits for Enter when interactive; removes leftovers of earlier runs; checked in CI, including a locked file; ships as `v0.3.0-rc.2` because `v0.3.0-rc.1` is already a published tag | brief 1 §8; D7, D53; matrix §7.10 SW-N4 |
...
- **Decision:**
  ...
  - **Leftovers.** After the refusal and running-copy checks, the script removes `DialShift.new-*` and `DialShift.old-*` folders that an earlier run left beside the install (best effort, errors ignored). A `DialShift.old-*` is kept while there is no `DialShift` install, because it is then the only copy of the previous install (a failed restore); the first run after a successful install removes it. A refused run removes nothing.
  ...
  - **CI check.** In `build.yml`, the Windows job runs a step "Install.ps1 (fresh install, upgrade, locked file, refusals)" after the zip verification. It runs the extracted zip's `Install.ps1` with `powershell.exe -NonInteractive -File … -NoLaunch`, with `LOCALAPPDATA` in a temp folder, and captures the output. The cases:
    - (a) A fresh install: the exe and the Start menu shortcut exist.
    - (b) An upgrade over it: a file only the old build had is gone, planted stale `DialShift.new-*`/`DialShift.old-*` folders are removed, `Programs` holds only `DialShift`, and an existing Run value now names the installed copy.
...
- **Consequence:** Evidence: `58b813a` and the review fixes in `dc032fe`. Matrix §7.10 SW-N4 is GREEN-pending-CI until the step's first green run.
  - **Covered on Windows by CI:** the success, locked-file (first rename fails, install unchanged) and refusal paths.
  - **Covered only by a PowerShell 7 harness on macOS with injected failures:** the restore after a failed second rename, a failed restore, a failed copy, a locked old copy (the warning), the rename retry, the leftover cleanup, and the interactive and non-interactive endings (under a pseudo-terminal).
