...
| D53 | 2026-09-25 | Slot times: `HH:mm` is canonical and written only by `Scheduler.FormatTime` (invariant); `TryTime` also accepts the culture-written `HH.mm`; `SettingsStore.Load` canonicalizes in memory; `Conflicts` compares parsed times | brief 1 §5 (Core), §7.9; brief 2 §7 (QA-N3) |
...
## D53 — Slot times: one canonical `HH:mm`, and the culture-written `HH.mm` still plays
