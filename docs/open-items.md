...
> **As of 2026-09-25**, after the pre-merge sweep, SW-N4, the first public push and the `v0.3.0` release. The public repository has `main` at `6d1a1cc`, `refactor/single-codebase-timezone` at `b9cbeea` (= `v0.3.0`) and the tags `v0.3.0-rc.1`, `v0.3.0-rc.2`, `v0.3.0` and `legacy-last-known-good`. **`v0.3.0` is published as the latest full release** (public Release run [`36172793355`](https://github.com/spyroskotsakis/DialShift/actions/runs/36172793355)), before the native checks (D57). The CI evidence below is private run `36163420184` at `bc807b5` and the public runs listed in [§2.10](#210-release-engineering-not-blocked-on-hardware-7-items); the private repository refuses every Actions job for billing, and the backup release path is `scripts/release-local.sh` (D58). Since 2026-09-26 the public repository also has `feature/add-station-catalog-search` at `cceff38`, green in CI [`36219966368`](https://github.com/spyroskotsakis/DialShift/actions/runs/36219966368), and `main` at `aaa3cb1`, whose CI `36219355899` failed the one Windows check that D91 fixes in `cceff38`.
> **Brief 3 (the station catalog search in the Add station dialog, `docs/add-station-catalog-search.md`), 2026-09-26:** implemented on `feature/add-station-catalog-search` (at `cceff38`) and prepared for release as `v0.4.0`, a full release before the by-hand checks (D92; CHANGELOG `[0.4.0]`; not tagged yet). Its acceptance rows, matrix §11 CAT-01..18, read 15 GREEN and 3 NATIVE-PENDING since the Windows CI run of [§2.12](#212-a-windows-ci-run-of-brief-3-done) (public CI `36219966368`); what it leaves open is NC-18 ([§2.11](#211-a-windows-machine-with-windows-powershell-51-1-item-brief-3)), NC-01's smoke from the extracted zip and its step (4b), step (6b) of NC-17, and the data decisions of [§2.13](#213-station-catalog-data-maintainer-decisions-brief-3).
...
> - [docs/decisions.md](decisions.md): decisions D1–D92 (brief 3 from D59).
>
...
**Missing resource:** only the private repository's billing is left, for the maintainer; the local backup release path (D58) works without it. SW-N4, the legacy tag, the public runs, Issues and the `v0.3.0` release are done; the `v0.4.0` release (D92) is next.
...
| Release `v0.4.0` (D92) | A full release on `cceff38`'s app code (the tag goes on a commit whose difference from `cceff38` is documentation only), with the testing status in CHANGELOG `[0.4.0]`, which cites CI run `36219966368`. **Public:** the maintainer pushes the tag `v0.4.0` to `origin` in a push of its own (README "First-push caveat"); `release.yml` runs `build.yml` on the tag and publishes only if every step passes. **Private:** `scripts/release-local.sh v0.4.0 --repo spyroskotsakis/dialshift-dev` as a dry run, then with `--publish` (D58), because the private repository's Actions are refused for billing. Public `main` is at `aaa3cb1`, whose CI `36219355899` failed the check D91 fixes; it is green again once it carries `cceff38` or later | Release, user decision (D92) | maintainer (tag, push, local release); release (notes) | The public Releases page shows `v0.4.0` as the latest release with `DialShift-win-x64.zip`, `DialShift-macos-arm64.zip` and `SHA256SUMS.txt`, and the checksums match; the private repository has its `v0.4.0` release from `release-local.sh`; both are recorded here and in D92 |
...
The [§2.10](#210-release-engineering-not-blocked-on-hardware-7-items) items need no hardware. All are done except the private repository's billing, which needs the maintainer (the local backup path, D58, releases without it). `v0.3.0` is released (D57), so the native checks above run against the released build and are tracked for 0.3.x; `v0.4.0` follows on the same basis (D92), and once it is published the Windows hardware block and NC-18 run from its `DialShift-win-x64.zip`, so that NC-01's smoke (36/36) and step (4b) cover the station catalog. Run the Windows hardware block from a zip built at or after `65771f7`, such as the `v0.3.0` release's `DialShift-win-x64.zip` (`b9cbeea`), so that NC-04 step 6 tests the fixed `Install.ps1`.
