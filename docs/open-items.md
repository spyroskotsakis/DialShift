...
> **Brief 4 (the Windows setup, `docs/windows-installer.md`), 2026-09-26: specified, not implemented.** An unsigned, per-user NSIS setup `DialShift-Setup-win-x64.exe` beside the unchanged zip and macOS download, on `feature/windows-installer` (D93–D99). Its rows INS-01..INS-20 (matrix §12) are all `TODO`; its by-hand check NC-19 ([§2.1](#21-windows-physical-hardware-7-items)) waits for a build that carries the setup.
...
> - [docs/decisions.md](decisions.md): decisions D1–D99 (brief 3 from D59, brief 4 from D93).
>
...
| Decisions | D1–D99 recorded (brief 3: D59–D92; brief 4, the Windows setup, spec only: D93–D99) | [docs/decisions.md](decisions.md) |
...
| Brief 4 (Windows setup) | **Specified, not implemented** (2026-09-26): `docs/windows-installer.md`, D93–D99; the brief's spike built an NSIS 3.12 setup from the `v0.4.0` package on the dev Mac (63.6 MB, two builds byte-identical) | [Matrix §12](acceptance-matrix.md#12-windows-installer-brief-4): 20 TODO |
...
- **§12 INS-11, INS-15, INS-16 and INS-20** (brief 4): need NC-19 once the setup is built (INS-11 also its CI case (s5); INS-16 also the design review).
...
- **Not in the kit:**
  ...
  - NC-19 (brief 4): the kit gains it with the setup's implementation (brief 4 §15, test lane).
