...
| Review findings (§7.10) | All GREEN or accepted except SR-02 and NX-01, both NATIVE-PENDING; LOG-D2 (a line lost in `dialshift.log` on a Windows runner, D103) is GREEN. The pre-merge sweep's blockers (SW-B1 RT-07 flake, SW-B2 culture-dependent slot times) and should-fixes (SW-S1..S5) are fixed, and SW-N4 (`Install.ps1`, D56) is GREEN in CI `36163420184` ([§2.10](#210-release-engineering-not-blocked-on-hardware-8-items)) | [Matrix §7.10](acceptance-matrix.md#710-characterization-and-review-findings-tracked) |
...
**Release status:** **`v0.4.1`** (`faaf781`), the fix for `https://` stations on Windows (D100), is the latest release on both repositories, published on 2026-09-26 on the same basis as `v0.4.0`: on the public repository by Release run [`36232921118`](https://github.com/spyroskotsakis/DialShift/actions/runs/36232921118) and on the private repository by `scripts/release-local.sh v0.4.1 --repo spyroskotsakis/dialshift-dev --publish` (D58), each with `DialShift-win-x64.zip`, `DialShift-macos-arm64.zip` and `SHA256SUMS.txt` ([§2.10](#210-release-engineering-not-blocked-on-hardware-8-items)). Its testing status is in the `[0.4.1]` section of `CHANGELOG.md` and in the README; its by-hand check NC-20 passed on the released `DialShift-win-x64.zip` in a Windows 11 ARM64 VM ([§2.14](#214-a-windows-machine-whose-root-store-can-be-edited-1-item-done-d100)). Before it, **`v0.4.0`** (`3c8543c`), the station catalog search, was published on 2026-09-26 on the same basis as `v0.3.0` (D92): on the public repository by Release run [`36221102328`](https://github.com/spyroskotsakis/DialShift/actions/runs/36221102328) with `DialShift-win-x64.zip`, `DialShift-macos-arm64.zip` and `SHA256SUMS.txt`, and on the private repository by `scripts/release-local.sh v0.4.0 --repo spyroskotsakis/dialshift-dev --publish` (D58) with the same three files ([§2.10](#210-release-engineering-not-blocked-on-hardware-8-items)). Its testing status is in the `[0.4.0]` section of `CHANGELOG.md` and in the README, and every check below stays open and is tracked for 0.4.x. Before those, **`v0.3.0`** (`b9cbeea`) was the first full release on the public repository, published on 2026-09-25 at 18:29 UTC by Release run [`36172793355`](https://github.com/spyroskotsakis/DialShift/actions/runs/36172793355) with `DialShift-win-x64.zip`, `DialShift-macos-arm64.zip` and `SHA256SUMS.txt` (D57). It has the same app code as the pre-release **`v0.3.0-rc.2`** (`99a1122`, the SW-N4 `Install.ps1` fix, D56), which follows **`v0.3.0-rc.1`** (`9b47afb`); both are published as pre-releases. Its testing status (Windows not yet tested by hand on a real PC; macOS tested on the developer's Mac only; unsigned) is in the `[0.3.0]` section of `CHANGELOG.md`, which is the release notes, and in the README. The private repository refuses every Actions job for billing, so its releases need the account fixed or the local backup path, `scripts/release-local.sh` (D58; [§2.10](#210-release-engineering-not-blocked-on-hardware-8-items)), which made its `v0.4.0` and `v0.4.1` releases. Releases follow the D53 pipeline (README "Releasing (maintainers)"). **`0.5.0`**, the Windows setup (brief 4), is prepared on `feature/windows-installer` (csproj `<Version>` 0.5.0 and CHANGELOG `[0.5.0]`, with its testing status: CI green, NC-19 by hand still to run) and is not tagged or published.
...
### 2.10 Release engineering, not blocked on hardware (8 items)

**Missing resource:** none for releasing. The private repository's Actions are still refused for billing (the maintainer's account), but the local backup release path (D58) works without them and made the private `v0.4.0` and `v0.4.1` releases. SW-N4, the legacy tag, the public runs, Issues and the `v0.3.0`, `v0.4.0` and `v0.4.1` releases are done.
...
The [§2.10](#210-release-engineering-not-blocked-on-hardware-8-items) items need no hardware. All are done; the private repository's Actions stay refused for billing until the maintainer fixes it, and the local backup path (D58) releases without them. `v0.3.0` (D57), `v0.4.0` (D92) and `v0.4.1` (D100) are released, so the native checks above run against a released build and are tracked for 0.4.x; run the Windows hardware block and NC-18 from the `DialShift-win-x64.zip` of `v0.4.1` or later, so that NC-01's smoke (36/36) and step (4b) cover the station catalog, and so that `https://` stations (the starter stations among them) play on a PC that lacks their root certificate: 0.4.0 fails them there (D100). NC-20 passed on `0.4.1+8742646` and on the `v0.4.1` release's zip (§2.14). Any zip built at or after `65771f7` (the `v0.3.0`, `v0.4.0` and `v0.4.1` releases' included) has the fixed `Install.ps1` that NC-04 step 6 tests.
