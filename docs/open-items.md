...
> **As of 2026-09-25**, after the pre-merge sweep, SW-N4, the first public push and the `v0.3.0` release. The public repository has `main` at `6d1a1cc`, `refactor/single-codebase-timezone` at `b9cbeea` (= `v0.3.0`) and the tags `v0.3.0-rc.1`, `v0.3.0-rc.2`, `v0.3.0` and `legacy-last-known-good`. **`v0.3.0` is published as the latest full release** (public Release run [`36172793355`](https://github.com/spyroskotsakis/DialShift/actions/runs/36172793355)), before the native checks (D57). The CI evidence below is private run `36163420184` at `bc807b5` and the public runs listed in [§2.10](#210-release-engineering-not-blocked-on-hardware-9-items); the private repository refuses every Actions job for billing, and the backup release path is `scripts/release-local.sh` (D58). Since 2026-09-26 the public repository also has `feature/add-station-catalog-search` and `main` at `3c8543c` (= `v0.4.0`), green in CI [`36221096207`](https://github.com/spyroskotsakis/DialShift/actions/runs/36221096207) and [`36221096133`](https://github.com/spyroskotsakis/DialShift/actions/runs/36221096133) (`main` at `aaa3cb1` had failed, in CI `36219355899`, the one Windows check that D91 fixes in `cceff38`), and **`v0.4.0` was published on both repositories** (public Release run [`36221102328`](https://github.com/spyroskotsakis/DialShift/actions/runs/36221102328); private by `scripts/release-local.sh`, D58; D92). **`v0.4.1`** (D100, the fix for `https://` stations on Windows) followed the same day: the tag and `main` at `faaf781` (public CI on `main` [`36232442733`](https://github.com/spyroskotsakis/DialShift/actions/runs/36232442733) green), public Release run [`36232921118`](https://github.com/spyroskotsakis/DialShift/actions/runs/36232921118), private by `scripts/release-local.sh` (D58); NC-20 passed on its released `DialShift-win-x64.zip` ([§2.14](#214-a-windows-machine-whose-root-store-can-be-edited-1-item-done-d100)). **`v0.5.0`** (brief 4, the Windows setup) followed the same day and **is the latest release on both repositories**: the tag and `main` at `7e08c3b` (public CI on `main` [`36246725331`](https://github.com/spyroskotsakis/DialShift/actions/runs/36246725331) green on `windows-latest`, `macos-latest` and the setup cross-build), public Release run [`36247644298`](https://github.com/spyroskotsakis/DialShift/actions/runs/36247644298), private by `scripts/release-local.sh` (D58), each with `DialShift-Setup-win-x64.exe` as a fourth asset ([§2.10](#210-release-engineering-not-blocked-on-hardware-9-items)).
> **Brief 3 (the station catalog search in the Add station dialog, `docs/add-station-catalog-search.md`), 2026-09-26:** implemented on `feature/add-station-catalog-search` (app code at `cceff38`) and released as **`v0.4.0`** (tag at `3c8543c`, published 2026-09-26), a full release before the by-hand checks (D92; CHANGELOG `[0.4.0]`; [§2.10](#210-release-engineering-not-blocked-on-hardware-9-items)). Its acceptance rows, matrix §11 CAT-01..18, read 15 GREEN and 3 NATIVE-PENDING since the Windows CI run of [§2.12](#212-a-windows-ci-run-of-brief-3-done) (public CI `36219966368`); what it leaves open is NC-18 ([§2.11](#211-a-windows-machine-with-windows-powershell-51-1-item-brief-3)), NC-01's smoke from the extracted zip and its step (4b), step (6b) of NC-17, and the data decisions of [§2.13](#213-station-catalog-data-maintainer-decisions-brief-3).
> **Brief 4 (the Windows setup, `docs/windows-installer.md`), 2026-09-26: implemented and released as `v0.5.0`** (the tag and `main` at `7e08c3b`, published 2026-09-26 on both repositories, a full release before the by-hand checks; CHANGELOG `[0.5.0]`; [§2.10](#210-release-engineering-not-blocked-on-hardware-9-items)). An unsigned, per-user NSIS setup `DialShift-Setup-win-x64.exe` beside the unchanged zip and macOS download, built on `feature/windows-installer` (D93–D99, D101, D104); public CI `36230181228` at `b655d67`, `36231552864` at `cf21fc1` and `36233711767` at `b7017d7` are green on both OSes and the cross-build job; `36238278091` at `b76356e`, `36240957220` at `6cbc311` (D101's last round), `36243801937` at `41a19de` (D104) and `36246725331` on `main` at `7e08c3b` green on all three jobs. Its rows INS-01..INS-20 (matrix §12): 13 GREEN, 4 NATIVE-PENDING (the rest of NC-19), 3 TODO (INS-13's R1 review, INS-15's review, INS-19's docs review); INS-17 and INS-18 turned GREEN with the `v0.5.0` releases; its by-hand check NC-19 ([§2.1](#21-windows-physical-hardware-7-items)) passed in part on 2026-09-26 in a Windows 11 ARM64 VM, not on a physical PC (matrix §9 lists what it covered).
...
> - [docs/decisions.md](decisions.md): decisions D1–D104 (brief 3 from D59 to D92; brief 4, the Windows setup, D93–D99, D101 and D104; D100, the 0.4.1 fix for `https://` stations on Windows; D102, CAT-16 at the JIT's steady state; D103, the log lock's wait, LOG-D2).
>
...
| Review findings (§7.10) | All GREEN or accepted except SR-02 and NX-01, both NATIVE-PENDING; LOG-D2 (a line lost in `dialshift.log` on a Windows runner, D103) is GREEN. The pre-merge sweep's blockers (SW-B1 RT-07 flake, SW-B2 culture-dependent slot times) and should-fixes (SW-S1..S5) are fixed, and SW-N4 (`Install.ps1`, D56) is GREEN in CI `36163420184` ([§2.10](#210-release-engineering-not-blocked-on-hardware-9-items)) | [Matrix §7.10](acceptance-matrix.md#710-characterization-and-review-findings-tracked) |
| Decisions | D1–D104 recorded (brief 3: D59–D92; brief 4, the Windows setup: D93–D99, D101 and D104 (the setup's own Welcome/Finish image); D100: the 0.4.1 fix for `https://` stations on Windows; D102: CAT-16 measured at the JIT's steady state, retried only on a busy machine; D103: the log lock waits while its holder makes progress, LOG-D2) | [docs/decisions.md](decisions.md) |
| Brief 3 (station catalog search) | Implemented; released as `v0.4.0` on 2026-09-26 (D92; public Release run [`36221102328`](https://github.com/spyroskotsakis/DialShift/actions/runs/36221102328), private release by `release-local.sh`, [§2.10](#210-release-engineering-not-blocked-on-hardware-9-items)). Local gate on the dev Mac (D75): build with warnings as errors clean, **2,702 passed / 6 skipped, 26/26 suites** (orchestrator at `8836ec3`, under `caffeinate -i`; the 6 SKIPs are Windows-only), the pipeline self-test 320/320, the bundle smoke 36/36, both package verifiers on the real packages (8,270 stations) and the 32 verifier fixtures on both halves. Windows and macOS CI: public [`36219966368`](https://github.com/spyroskotsakis/DialShift/actions/runs/36219966368) at `cceff38` green on both OSes (**windows-latest 2,627 passed / 25 skipped, macos-latest 2,704 passed / 6 skipped, 26/26 suites**), after D91 fixed the one Windows failure of the first run | [Matrix §11](acceptance-matrix.md#11-add-station-catalog-search-brief-3): 15 GREEN, 3 NATIVE-PENDING (CAT-03, CAT-12, CAT-14), 0 WINDOWS-PENDING, 0 TODO |
| Brief 4 (Windows setup) | **Implemented; released as `v0.5.0` on 2026-09-26** (public Release run [`36247644298`](https://github.com/spyroskotsakis/DialShift/actions/runs/36247644298), private release by `release-local.sh`, [§2.10](#210-release-engineering-not-blocked-on-hardware-9-items)): `docs/windows-installer.md`, D93–D99, D101, D104; public CI `36230181228` at `b655d67`, `36231552864` at `cf21fc1` and `36233711767` at `b7017d7` green on both OSes, the Mac-built and Windows-built setups byte-identical; `36238278091` at `b76356e`, `36240957220` at `6cbc311` (D101's last round), `36243801937` at `41a19de` and [`36246725331`](https://github.com/spyroskotsakis/DialShift/actions/runs/36246725331) on `main` at `7e08c3b` green on all three jobs | [Matrix §12](acceptance-matrix.md#12-windows-installer-brief-4): 13 GREEN, 4 NATIVE-PENDING, 3 TODO; NC-19 PASS (partial, a VM) |
...
**Release status:** **`v0.5.0`** (`7e08c3b`), the Windows setup (brief 4, D93–D99, D101, D104) with the D103 log fix, is the latest release on both repositories, published on 2026-09-26 on the same basis as `v0.4.1`: on the public repository by Release run [`36247644298`](https://github.com/spyroskotsakis/DialShift/actions/runs/36247644298) and on the private repository by `scripts/release-local.sh v0.5.0 --repo spyroskotsakis/dialshift-dev --publish` (D58), each with `DialShift-win-x64.zip`, `DialShift-Setup-win-x64.exe`, `DialShift-macos-arm64.zip` and `SHA256SUMS.txt` (three lines, one per download; [§2.10](#210-release-engineering-not-blocked-on-hardware-9-items)). Its testing status is in the `[0.5.0]` section of `CHANGELOG.md` and in the README; its by-hand check NC-19 has passed only in part, in a Windows 11 ARM64 VM ([§2.1](#21-windows-physical-hardware-7-items)), and runs next from the released setup. Before it, **`v0.4.1`** (`faaf781`), the fix for `https://` stations on Windows (D100), was published on 2026-09-26 on the same basis as `v0.4.0`: on the public repository by Release run [`36232921118`](https://github.com/spyroskotsakis/DialShift/actions/runs/36232921118) and on the private repository by `scripts/release-local.sh v0.4.1 --repo spyroskotsakis/dialshift-dev --publish` (D58), each with `DialShift-win-x64.zip`, `DialShift-macos-arm64.zip` and `SHA256SUMS.txt` ([§2.10](#210-release-engineering-not-blocked-on-hardware-9-items)). Its testing status is in the `[0.4.1]` section of `CHANGELOG.md` and in the README; its by-hand check NC-20 passed on the released `DialShift-win-x64.zip` in a Windows 11 ARM64 VM ([§2.14](#214-a-windows-machine-whose-root-store-can-be-edited-1-item-done-d100)). Before it, **`v0.4.0`** (`3c8543c`), the station catalog search, was published on 2026-09-26 on the same basis as `v0.3.0` (D92): on the public repository by Release run [`36221102328`](https://github.com/spyroskotsakis/DialShift/actions/runs/36221102328) with `DialShift-win-x64.zip`, `DialShift-macos-arm64.zip` and `SHA256SUMS.txt`, and on the private repository by `scripts/release-local.sh v0.4.0 --repo spyroskotsakis/dialshift-dev --publish` (D58) with the same three files ([§2.10](#210-release-engineering-not-blocked-on-hardware-9-items)). Its testing status is in the `[0.4.0]` section of `CHANGELOG.md` and in the README, and every check below stays open and is tracked for 0.4.x. Before those, **`v0.3.0`** (`b9cbeea`) was the first full release on the public repository, published on 2026-09-25 at 18:29 UTC by Release run [`36172793355`](https://github.com/spyroskotsakis/DialShift/actions/runs/36172793355) with `DialShift-win-x64.zip`, `DialShift-macos-arm64.zip` and `SHA256SUMS.txt` (D57). It has the same app code as the pre-release **`v0.3.0-rc.2`** (`99a1122`, the SW-N4 `Install.ps1` fix, D56), which follows **`v0.3.0-rc.1`** (`9b47afb`); both are published as pre-releases. Its testing status (Windows not yet tested by hand on a real PC; macOS tested on the developer's Mac only; unsigned) is in the `[0.3.0]` section of `CHANGELOG.md`, which is the release notes, and in the README. The private repository refuses every Actions job for billing, so its releases need the account fixed or the local backup path, `scripts/release-local.sh` (D58; [§2.10](#210-release-engineering-not-blocked-on-hardware-9-items)), which made its `v0.4.0`, `v0.4.1` and `v0.5.0` releases. Releases follow the D53 pipeline (README "Releasing (maintainers)").
...
- the release-engineering and maintainer items in [§2.10](#210-release-engineering-not-blocked-on-hardware-9-items), which need no hardware.

Native checks: 18 open. NC-01..NC-13 and NC-15..NC-19, where NC-07, NC-13, NC-17 and NC-19 are partly done; NC-18 comes from brief 3 (the catalog search, D82) and needs only a Windows machine with Windows PowerShell 5.1 ([§2.11](#211-a-windows-machine-with-windows-powershell-51-1-item-brief-3)); NC-19 comes from brief 4 (the Windows setup, D93–D99, D101) and passed in part on 2026-09-26 in a Windows 11 ARM64 VM (a fresh install, SmartScreen, the wizard at 200 %, the uninstaller's running check and settings question, a silent reinstall), not on a physical PC, from a local build, since no release carried the setup yet; its next run downloads the `v0.5.0` release's `DialShift-Setup-win-x64.exe`; the `0.4.0` `Install.ps1` upgrade with launch at sign-in, keyboard navigation, the Settings → Apps page, the setup's own running check, Windows 10 and a physical PC remain. NC-20, added for the 0.4.1 fix for `https://` stations on Windows (D100), is PASS: the full row passed by hand on 2026-09-26 on `0.4.1+8742646` and again on the `v0.4.1` release's `DialShift-win-x64.zip` (`0.4.1+faaf781`), in a Windows 11 ARM64 VM, not on a physical PC ([§2.14](#214-a-windows-machine-whose-root-store-can-be-edited-1-item-done-d100)). NC-14 (Intel) is not applicable (D13).
...
| NC-19 | The Windows setup (brief 4) on a real PC: download with a browser, SmartScreen, Smart App Control and Defender recorded; the wizard; the refusal while DialShift runs; an upgrade of a `0.4.0` `Install.ps1` install with launch at sign-in on; Apps & features; reinstall, upgrade, and uninstall with both answers to the keep-settings question | Needs a real Windows PC, a person at the wizard and a real sign-in; CI runs the setup silently only. **PASS (partial), 2026-09-26, in a Windows 11 ARM64 VM, not a physical PC** (matrix §9 NC-19 has the coverage): a fresh install, SmartScreen (**Run anyway**, Smart App Control off, no Defender verdict), the wizard at 200 %, the installed app playing, the uninstall entry's values, the uninstaller's running check and settings question (Yes and No), a silent reinstall. Still open: the `0.4.0` `Install.ps1` upgrade with launch at sign-in on, a sign-out and in, keyboard navigation, the Settings → Apps page, an upgrade from an older setup, the setup's own running check, the D104 Welcome/Finish image, Windows 10, a physical PC | §9 NC-19, steps 1–8 | SmartScreen and the rest recorded; the wizard reads right at 150 %; the Retry/Cancel message; stations, schedule and launch at sign-in (on, no diagnostic) kept; the installed exe starts without a second SmartScreen prompt; Apps & features shows DialShift, its version, publisher `DialShift`, icon and size; Yes keeps and No deletes `%LOCALAPPDATA%\DialShift`; shortcuts and the Run value removed | Matrix §12 INS-07, INS-11, INS-15, INS-16, INS-20 |
...
### 2.10 Release engineering, not blocked on hardware (9 items)

**Missing resource:** none for releasing. The private repository's Actions are still refused for billing (the maintainer's account), but the local backup release path (D58) works without them and made the private `v0.4.0`, `v0.4.1` and `v0.5.0` releases. SW-N4, the legacy tag, the public runs, Issues and the `v0.3.0`, `v0.4.0`, `v0.4.1` and `v0.5.0` releases are done.
...
| Release `v0.4.1` (D100) | **Done on 2026-09-26.** A full release on `8742646`'s app code (the D100 fix): the tag `v0.4.1` is on `faaf781`, whose difference from `8742646` is documentation only, with the testing status in CHANGELOG `[0.4.1]`; `main` on both repositories is `faaf781`, and public CI on it is green ([`36232442733`](https://github.com/spyroskotsakis/DialShift/actions/runs/36232442733)). **Public** ([release](https://github.com/spyroskotsakis/DialShift/releases/tag/v0.4.1)): the tag push started Release run [`36232921118`](https://github.com/spyroskotsakis/DialShift/actions/runs/36232921118). Its first attempt passed "Check the tag, version and changelog" and `build / windows-latest` (`2678 passed, 25 skipped; 27/27 suites green.`), but `build / macos-latest` failed in its test step on one timing gate only, `FAIL: CAT-16 load median (synthetic 10000) 67.1 ms < 50 ms` (`2731 passed, 6 skipped; 26/27 suites green.`), so publishing was skipped; the six public CI runs before it measured 33.5–43.5 ms on that gate, and the catalog loading code is unchanged since `v0.4.0`. The failed job was re-run: attempt 2 passed (`26.3 ms < 50 ms`; `2738 passed, 6 skipped; 27/27 suites green.`), and "Publish the GitHub Release" published "DialShift 0.4.1" at 09:47:02 UTC, not a pre-release, as Latest, with `DialShift-win-x64.zip` (97,935,376 B, SHA-256 `56206217…8cb3ff54`), `DialShift-macos-arm64.zip` (45,665,003 B, `2e6c66ad…538b702c`) and `SHA256SUMS.txt`, whose lines equal the digests GitHub reports for the two zips. **Private** ([release](https://github.com/spyroskotsakis/dialshift-dev/releases/tag/v0.4.1)): the maintainer ran `scripts/release-local.sh v0.4.1 --repo spyroskotsakis/dialshift-dev --publish` on the Mac (D58): build with warnings as errors, tests `2738 passed, 6 skipped` (27/27 suites), the Windows package cross-built with `build.ps1` and verified, the macOS zip verified after `ditto` and `unzip`, native smoke 36/36, bundle smoke 34/34 (`MacAvPlayerPlaybackEngine`), verifier fixtures 32/32 on both packages; "DialShift 0.4.1" was published at 09:36:29 UTC, not a pre-release, as Latest, with `DialShift-win-x64.zip` (97,933,667 B, `4f2e3b6c…0f3e1df5`), `DialShift-macos-arm64.zip` (45,665,016 B, `12eb5b09…c1e4c2cd`) and a matching `SHA256SUMS.txt`. Its notes say what was checked on the Mac and what was not: the Windows-only tests, the Windows native smoke and `Install.ps1`, because they need Windows; they ran in the public Release run. The two repositories' zips differ in bytes: separate builds of the same commit. NC-20 passed again on the public release's `DialShift-win-x64.zip` ([§2.14](#214-a-windows-machine-whose-root-store-can-be-edited-1-item-done-d100)). Follow-up, since done: CAT-16's load-time gate is measured at the JIT's steady state and retried only on a busy runner (D102, merged to `main` at `df9c232`, in `v0.5.0`) | Was: release (D100) | maintainer (tag, push, local release); release (notes) | Done: both Releases pages show `v0.4.1` as the latest release with the three assets, the checksums match, and D100's update records both |
| Release `v0.5.0` (brief 4, D93–D99, D101, D104) | **Done on 2026-09-26.** The first release with the Windows setup, and the D103 log fix: the tag `v0.5.0` and `main` on both repositories are `7e08c3b` (the merge of `feature/windows-installer`), with the testing status in CHANGELOG `[0.5.0]`; public CI on `main` at `7e08c3b` is green on all three jobs ([`36246725331`](https://github.com/spyroskotsakis/DialShift/actions/runs/36246725331)). **Public** ([release](https://github.com/spyroskotsakis/DialShift/releases/tag/v0.5.0)): the tag push started Release run [`36247644298`](https://github.com/spyroskotsakis/DialShift/actions/runs/36247644298), which passed "Check the tag, version and changelog", `build / macos-latest` (`2755 passed, 6 skipped; 27/27 suites green.`, bundle smoke `34/34`), `build / windows-latest` (`2695 passed, 25 skipped; 27/27 suites green.`, the setup built with `nsis-3.12.zip`, `verify-win-setup.sh --require-contents`, `All 6 setup fixtures got the expected verdict.` and the setup cases (s1)–(s9)), `build / Setup cross-build (macOS)` (`Verdict: **byte-identical (D98)**`: the Windows-built and Mac-built setups both `bd1de1b0…e9a3a8bd`) and "Publish the GitHub Release", all on the first attempt; "DialShift 0.5.0" was published at 14:26:02 UTC, not a pre-release, as Latest, with `DialShift-win-x64.zip` (97,940,242 B, SHA-256 `7c22c640…f637287a`), `DialShift-Setup-win-x64.exe` (63,737,320 B, `bd1de1b0…e9a3a8bd`), `DialShift-macos-arm64.zip` (45,666,348 B, `2f54fddf…2923c35c`) and `SHA256SUMS.txt` (three lines); `shasum -a 256 -c SHA256SUMS.txt` on the three downloads prints `OK` for each (this Mac, 2026-09-26). **Private** ([release](https://github.com/spyroskotsakis/dialshift-dev/releases/tag/v0.5.0)): published from the Mac with `scripts/release-local.sh v0.5.0 --repo spyroskotsakis/dialshift-dev --publish` (D58), whose run did: build with warnings as errors, tests `2755 passed, 6 skipped; 27/27 suites green.`, the Windows package cross-built with `build.ps1` (`0.5.0+7e08c3b…`) and verified after extraction, the setup built with Homebrew `makensis` `v3.12` from the uploaded zip and checked by `verify-win-setup.sh` (static checks; contents not checked, 7-Zip not installed), the macOS zip verified after `ditto` and `unzip`, native smoke `36/36`, bundle smoke `34/34` (`MacAvPlayerPlaybackEngine`), `All 32 fixtures got the expected verdict.` and `All 6 setup fixtures got the expected verdict.`; "DialShift 0.5.0" was published at 14:19:29 UTC, not a pre-release, as Latest, with `DialShift-win-x64.zip` (97,938,543 B, `6c8d431a…a9b73fa7`), `DialShift-Setup-win-x64.exe` (63,727,526 B, `f04d0a1e…50000e5f`), `DialShift-macos-arm64.zip` (45,666,356 B, `1504477b…7159318c`) and `SHA256SUMS.txt`, whose three lines `shasum -c` accepts on the downloads. Its notes list what was not checked on the Mac: the tests' Windows-only checks, the Windows native smoke and the `Install.ps1` checks, the setup's install, upgrade and uninstall cases, and the setup's contents; for the public build, all of them ran in the public Release run. The two repositories' files differ in bytes (separate builds of the same commit; the private setup packs the private zip). NC-19 has not run on a released setup yet ([§2.1](#21-windows-physical-hardware-7-items)) | Was: release (brief 4 §9, D99) | maintainer (tag, push, local release); release (notes) | Done: both Releases pages show `v0.5.0` as the latest release with the four assets, the checksums match, D99's update records both, and matrix §12 INS-17 and INS-18 are GREEN |
...
### 2.14 A Windows machine whose root store can be edited (1 item, done, D100)

**Done on 2026-09-26, by hand, in a virtual machine, not on a physical PC:** first on `0.4.1+8742646` built with `scripts/build.ps1`, then on the `v0.4.1` release's `DialShift-win-x64.zip`. The hosted `windows-latest` runners have every root installed, which is why CI never showed the defect D100 fixes (0.4.0 played no `https://` station on a Windows whose root store lacked the station's root), and this Mac has no Windows root store. The full evidence is in matrix §9 NC-20.

| Item | What's left | Result | Procedure | Pass (short) | Rows it unblocks |
|---|---|---|---|---|---|
| NC-20 | Nothing | **PASS.** Windows 11 ARM64 in a Parallels VM (OS version 10.0.26100, the x64 app under emulation). **From the release:** the public `v0.4.1` release's `DialShift-win-x64.zip` (its digest checked against the release's `SHA256SUMS.txt`), ProductVersion `0.4.1+faaf781`: USERTrust RSA removed from `Cert:\LocalMachine\AuthRoot` (`LocalMachine\Root` count `0`); "Listen to Groove Salad" at 09:48:10.53Z, event 4097 for USERTrust RSA at 09:48:11.37Z, `Connecting -> Playing; station='Groove Salad'; session=1` at 09:48:12.66Z (the first attempt); Groove Salad, Drone Zone, Secret Agent and the catalog station Radio BOB Progressive Rock (put into `settings.json` with its catalog URL), 3 starts each: 12 of 12 reached Playing; `dialshift.log` has 0 `playback.failed`, 0 `TlsFailure` and 0 `playback.tls_warmup` lines; `LocalMachine\Root` count `1` afterwards. **Before the release:** DialShift `0.4.1+8742646` built with `scripts/build.ps1`, package verified. USERTrust RSA removed from `LocalMachine\AuthRoot` (`LocalMachine\Root` and `CurrentUser\Root` count 0). The first Groove Salad start reached Playing on its first attempt 2.4 s after Listen, with event 4097 (the automatic update of USERTrust RSA) in between; Groove Salad, Drone Zone, Secret Agent and the catalog station Radio BOB Progressive Rock (`https://`, put into `settings.json` with its catalog URL, not through the Add-station dialog), 3 rounds by UI Automation: 12 of 12 starts reached Playing. `dialshift.log`: 0 `playback.failed`, 0 `TlsFailure`, 0 `playback.tls_warmup`. The root was back afterwards (count 1). Same VM, root removed, 0.4.0: 0 plays, 37 `TlsFailure` | §9 NC-20 | SomaFM and a catalog station play on the first attempt; no `TlsFailure` and no `playback.tls_warmup` warning; event 4097 adds the root; it is back in the store | D100's by-hand verification (done); MX-10 and MX-13 still need NC-03 and NC-15 (MX-13 also NC-11, NC-16) |


### 2.15 The catalog's cold first load is not gated (1 follow-up, brief 3)

**Missing resource:** a decision; no hardware is needed. Since D102, CAT-16 gates the catalog load at the JIT's steady state, which no longer depends on the suite order. The app, however, loads the catalog once per process, cold (D60, D80): the first time an Add dialog opens. CAT-16 prints that cold first load and does not gate it (D69, D80).

**Why it matters.** QA showed that the gate no longer notices a cold-start regression. With D80's six `[MethodImpl(MethodImplOptions.AggressiveOptimization)]` attributes removed from `DialShift.App/Services/CatalogProvider.cs` (lines 372–462), the cold first load rose to 49.5 and 50.4 ms, at the budget. The gated steady-state median stayed about 13 ms, and CAT-16 still passed.

| Item | What's left | Severity | Owner | Done when |
|---|---|---|---|---|
| Guard the cold load (D102 follow-up) | Choose and implement one guard for the load the user actually waits for. **Option 1:** gate the cold first load, reported by CAT-16, on local Apple Silicon Release runs. It is the first load in a standalone `--filter CatalogPerf` process; hosted runners would only report it, since their cold load was 78.7 ms in the flaked run. **Option 2:** gate the bundle smoke's `catalog.loaded` on local Apple Silicon (NC-17 already runs it). **Option 3:** gate the ratio of the cold load to the steady-state median, which does not depend on the machine's speed. The choice needs a decision, since D69 reports the cold load and does not gate it. Whatever is chosen, removing the `AggressiveOptimization` attributes must fail it | Follow-up, not a release blocker: CAT-16's budgets are met, and the cold load measured 39–44 ms standalone on the M4 Max at `68b292d` | spec lane (decision), then the test lane | A decision records the guard, and a mutation check without the attributes fails it |
...
The [§2.10](#210-release-engineering-not-blocked-on-hardware-9-items) items need no hardware. All are done; the private repository's Actions stay refused for billing until the maintainer fixes it, and the local backup path (D58) releases without them. `v0.3.0` (D57), `v0.4.0` (D92), `v0.4.1` (D100) and `v0.5.0` (brief 4) are released, so the native checks above run against a released build and are tracked for 0.5.x; run the Windows hardware block and NC-18 from the `DialShift-win-x64.zip` of `v0.4.1` or later, so that NC-01's smoke (36/36) and step (4b) cover the station catalog, and so that `https://` stations (the starter stations among them) play on a PC that lacks their root certificate: 0.4.0 fails them there (D100). NC-20 passed on `0.4.1+8742646` and on the `v0.4.1` release's zip (§2.14). Run NC-19 from the `DialShift-Setup-win-x64.exe` of `v0.5.0` or later, downloaded with a browser, so that it records the release it came from. Any zip built at or after `65771f7` (the `v0.3.0`, `v0.4.0`, `v0.4.1` and `v0.5.0` releases' included) has the fixed `Install.ps1` that NC-04 step 6 tests.
...
**Release versus sign-off (D57).** By the user's decision D57, `v0.3.0` shipped as a full release **before** these criteria hold (published 2026-09-25, public Release run `36172793355`), with its testing status in the release notes and the README. That overrides, for 0.3.0 only, the rule that a full release waits for the native checks and signing; D92 does the same for 0.4.0 (published 2026-09-26, public Release run `36221102328`, and on the private repository by `release-local.sh`), and 0.4.1 (D100) and 0.5.0 (brief 4, the Windows setup) follow on the same basis (published 2026-09-26, public Release runs `36232921118` and `36247644298`, and on the private repository by `release-local.sh`). None of them changes the criteria below: they still decide when the phase is signed off, and every open item stays tracked for 0.5.x.

**The phase is signed off when both of these hold:**
1. Every item in this file is PASS and recorded in [matrix §9](acceptance-matrix.md#9-remaining-native-checks), its [§2.9](#29-follow-ups-that-depend-on-the-above) follow-ups are done, and the [§2.10](#210-release-engineering-not-blocked-on-hardware-9-items) items are done.
