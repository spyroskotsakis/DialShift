...
> **Brief 3 (the station catalog search in the Add station dialog, `docs/add-station-catalog-search.md`), 2026-09-26:** implemented on `feature/add-station-catalog-search` (at `11114fb`) and not released (CHANGELOG `[Unreleased]`). Its acceptance rows, matrix §11 CAT-01..18, read 10 GREEN and 8 WINDOWS-PENDING (D75); what it leaves open is in [§2.11](#211-a-windows-machine-with-windows-powershell-51-1-item-brief-3), [§2.12](#212-a-windows-ci-run-of-brief-3-8-rows) and [§2.13](#213-station-catalog-data-maintainer-decisions-brief-3), plus step (4b) of NC-01 and step (6b) of NC-17.
...
> - [docs/decisions.md](decisions.md): decisions D1–D90 (brief 3 from D59).
>
...
| Decisions | D1–D90 recorded (brief 3: D59–D90) | [docs/decisions.md](decisions.md) |
| Brief 3 (station catalog search) | Implemented, not released. Local gate on the dev Mac (D75): build with warnings as errors clean, **2,694 passed / 6 skipped, 26/26 suites** (orchestrator at `0409c25`; the 6 SKIPs are Windows-only; 2,702 / 6 at `f45aeeb` with the last two checks, test lane, 3 runs), the pipeline self-test 320/320, the bundle smoke 36/36, both package verifiers on the real packages (8,270 stations) and the 32 verifier fixtures on both halves | [Matrix §11](acceptance-matrix.md#11-add-station-catalog-search-brief-3): 10 GREEN, 8 WINDOWS-PENDING, 0 TODO |
...
- **§11 CAT-03** (brief 3): `WINDOWS-PENDING` (D75); it needs NC-18 and the Windows native smoke from the zip ([§2.12](#212-a-windows-ci-run-of-brief-3-8-rows)).
- **§11 CAT-12 and CAT-14** (brief 3): `WINDOWS-PENDING`; besides the `windows-latest` headless run they name NC-01 step (4b) (Narrator, focus outlines, Segoe UI widths, real logos). NC-17 step (6b) is their macOS by-hand pass.
...
### 2.12 A Windows CI run of brief 3 (8 rows)

**Missing resource:** one green run of `build.yml` on `windows-latest` at the brief 3 branch. The private repository refuses every Actions job for billing (§2.10), and brief 3 does not push to the public repository. D75 makes the local macOS build plus tests the gate meanwhile, so the rows below are `WINDOWS-PENDING`: their macOS evidence is complete and only this run (or a Windows machine) is missing.

**Who can unblock:** the maintainer, by fixing the private repository's billing, pushing the branch there (`git push private feature/add-station-catalog-search`, which starts CI) and re-running CI if needed (`gh workflow run CI --ref feature/add-station-catalog-search -R spyroskotsakis/dialshift-dev`), or by pushing the branch to the public repository, where Actions are free (a maintainer step; agents never push to `origin`).

| Item | What's left | Why blocked | Procedure | Pass (short) | Rows it unblocks |
|---|---|---|---|---|---|
| CI-B3-WIN | The `windows-latest` job of `build.yml` at the branch: the build with warnings as errors and `dotnet publish -r win-x64` (the catalog next to `DialShift.exe`), `DialShift.Tests` (including the Windows-only checks, and CAT-05's drive and UNC rules that SKIP on macOS), the native smoke with its "Catalog loads from the app folder" check, `verify-win-package.ps1` on the extracted zip, and the step `Package verifier fixtures (win-x64, CAT-03)` | Needs Windows; the private repository's Actions are refused for billing | The CI workflow on the branch; read the job log and the `smoke-win-x64` artifact | Every step green; the smoke's `results.json` all passed with the catalog check; 32/32 fixture verdicts | CAT-02 (build and publish), CAT-03 (with NC-18), CAT-04 (the smoke's catalog check), CAT-05 (Windows path rules), CAT-06 (OS normalization data), CAT-11 (headless), CAT-12 (headless, with NC-01 step (4b)), CAT-14 (headless, with NC-01 step (4b)) |

When the run is green, flip each row whose Status cell names only this run (CAT-02, CAT-04, CAT-05, CAT-06, CAT-11) to `GREEN` in matrix §11, with the run id; CAT-03 also waits for NC-18, and CAT-12 and CAT-14 for NC-01 step (4b). The run also settles brief §10's definition of done ("the 24 existing suites plus the new `Catalog` suite pass on both CI OSes") for the "DoD only" rows.
...
2. The matrix reads **106 GREEN / 0 NATIVE-PENDING / 0 TODO** in §3, with QA-N1, TZ-12, SR-02, NX-01 and SW-N4 GREEN, and, for brief 3, §11 reads **18 GREEN** (the CI run of [§2.12](#212-a-windows-ci-run-of-brief-3-8-rows), NC-18, and NC-01 step (4b)). The data decisions of [§2.13](#213-station-catalog-data-maintainer-decisions-brief-3) do not gate sign-off.
