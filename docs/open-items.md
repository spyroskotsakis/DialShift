...
### 2.14 A Windows machine whose root store can be edited (done, D100)
...
| Item | What was left | Result | Procedure | Pass (short) | Rows it unblocked |
|---|---|---|---|---|---|
| NC-20 | Remove the USERTrust RSA root (thumbprint `2B8F1B57…DAB9AD8E`, exported first) from the store that holds it, then play the SomaFM starter stations | **PASS.** Windows 11 ARM64 in a Parallels VM (build 26100, the x64 app under emulation). The root removed from `LocalMachine\AuthRoot` (`LocalMachine\Root` view count 0). Build `0.4.1+0a472f6`: 12 of 12 SomaFM starts played (Drone Zone, Secret Agent, Groove Salad ×4), 0 `TlsFailure`; the root was back afterwards (view count 1). Same VM, root removed, 0.4.0: 0 plays, 37 `TlsFailure` | §9 NC-20 | SomaFM plays on its first attempt; no `TlsFailure`; the root is back in the store | None on its own: MX-10 and MX-13 still wait for NC-03 and NC-15 (MX-13 also NC-11, NC-16). It verifies D100 by hand |
