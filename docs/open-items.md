...
> **Brief 3 (the station catalog search in the Add station dialog, `docs/add-station-catalog-search.md`), 2026-09-26:** implemented on `feature/add-station-catalog-search` (at `11114fb`) and prepared for release as `v0.4.0` (CHANGELOG `[0.4.0]`; not tagged yet). Its acceptance rows, matrix §11 CAT-01..18, read 10 GREEN and 8 WINDOWS-PENDING (D75); what it leaves open is in [§2.11](#211-a-windows-machine-with-windows-powershell-51-1-item-brief-3), [§2.12](#212-a-windows-ci-run-of-brief-3-8-rows) and [§2.13](#213-station-catalog-data-maintainer-decisions-brief-3), plus step (4b) of NC-01 and step (6b) of NC-17.
