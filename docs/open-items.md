...
| Brief 3 (station catalog search) | Implemented, not released. Local gate on the dev Mac (D75): build with warnings as errors clean, **2,694 passed / 6 skipped, 26/26 suites** (orchestrator at `0409c25`; the 6 SKIPs are Windows-only), the pipeline self-test 320/320, the bundle smoke 36/36, both package verifiers on the real packages (8,270 stations) and the 32 verifier fixtures on both halves | [Matrix §11](acceptance-matrix.md#11-add-station-catalog-search-brief-3): 10 GREEN, 8 WINDOWS-PENDING, 0 TODO |
