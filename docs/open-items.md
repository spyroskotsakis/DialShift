...
> **Brief 4 (the Windows setup, `docs/windows-installer.md`), 2026-09-26: implemented; prepared for release as `0.5.0`, not tagged or published.** An unsigned, per-user NSIS setup `DialShift-Setup-win-x64.exe` beside the unchanged zip and macOS download, on `feature/windows-installer` (D93–D99, D101); public CI `36230181228` at `b655d67`, `36231552864` at `cf21fc1` and `36233711767` at `b7017d7` are green on both OSes and the cross-build job; `36238278091` at `b76356e` and `36240957220` at `6cbc311` (D101's last round) green on all three jobs. Its rows INS-01..INS-20 (matrix §12): 11 GREEN, 5 NATIVE-PENDING (NC-19), 4 TODO (INS-13's R1 review, INS-17 and INS-18 at the first release with the setup, `0.5.0`, INS-19's docs review); its by-hand check NC-19 ([§2.1](#21-windows-physical-hardware-7-items)) is open.
...
> - [docs/decisions.md](decisions.md): decisions D1–D102 (brief 3 from D59 to D92; brief 4, the Windows setup, D93–D99 and D101; D100, the 0.4.1 fix for `https://` stations on Windows; D102, CAT-16 at the JIT's steady state).
>
...
| Decisions | D1–D102 recorded (brief 3: D59–D92; brief 4, the Windows setup: D93–D99 and D101; D100: the 0.4.1 fix for `https://` stations on Windows; D102: CAT-16 measured at the JIT's steady state, retried only on a busy machine) | [docs/decisions.md](decisions.md) |
