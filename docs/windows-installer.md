...
> **Status: spec, 2026-09-26, not implemented.** Branch `feature/windows-installer` (from `main` at `fb0ef0b`).
...
4. **Setup (silent install, upgrade, Install.ps1 coexistence, refusals, uninstall):** pwsh, 10-minute timeout, with
   ...
   - **(s9) Setup over an `Install.ps1` install:** `Install.ps1 -NoLaunch` into the same folder, then the setup `/S`:
     ...
     value pointing at another copy is **kept** byte for byte.
...
| 2 | A running DialShift: refuse or close it? | Refuse with Retry/Cancel (silent: exit 2); never close or kill it | D94 |
