...
> `36231552864`, the Mac-built and Windows-built setups byte-identical) at `b7017d7` (run `36233711767`) and, on `windows-latest`,
> at `9322106` (run `36235173662`, all three jobs green after a re-run), with D101's sixth round pending its run; NC-19
...
- **Refusals first:** R2 (the install lock, exit 11), `CheckFolderPath` as in R3 (b) (exit 13, D101): a drive or share
  ...
  folder. …`; exit 13); then the folder must be the install this uninstaller belongs to: it must hold `Uninstall
  DialShift.exe`, and a temporary copy (the relaunch from `$TEMP\~nsu<n>.tmp`) must be as large as that file (exit 13,
  `This uninstaller won't remove files from <folder>: that isn't the DialShift install it belongs to. …`). An
  uninstaller started without `_?=` runs as a first process with no script, which NSIS also gives a `/D=`, and
  relaunches its copy with that folder already cleaned (`/D=C:\t/Victim` arrives as `C:\tVictim`), so the `_?=` text
  check cannot see it there; the install check does. Remaining gap: a `/D=` that NSIS cleans into another DialShift
  install of the same build uninstalls that install. And R5 (DialShift running, Retry/Cancel, exit 10; 15 when it
