...
> `docs/decisions.md` (D59–D68 are the brief's §11 defaults, D69–D76 the ambiguities resolved in Phase 0, D77–D82
...
> no https → http redirect, the verifiers' scope), which fix round 2 implements.
...
| Four concurrent 4096 × 4096 decodes against one at a time (fix round 2) | about **410 MiB** retained against about **152 MiB** | Decodes serialized by a single-slot gate, `MaxPixels` unchanged (D82 (a)) |
