...
> `docs/decisions.md` (D59–D68 are the brief's §11 defaults, D69–D76 the ambiguities resolved in Phase 0, D77–D87
...
> `DsMutedBrush`, accepted), its new §8 checks pending (test lane). D86 (readable tag notes and one spelling per place in
...
> (CAT-12, CAT-13, CAT-14); **not implemented yet** (UI lane round 2, then the test lane).
...
public sealed class StationEditorViewModel : EditorViewModel
{
    ...
    public string StatusLineText { get; }                        // D87: HasNoMatches && catalogEntryCount > 0 ? CatalogNoMatch
                                                                 // : CatalogStatusText (the status line binds it)
...
- **Status line** (D87): `StatusLineText` is `UiText.CatalogNoMatch` while `HasNoMatches` and the loaded catalog has at least one entry (`catalogEntryCount`, the loaded index's `Entries.Count`; 0 before the load and when Unavailable), otherwise `CatalogStatusText`; so a loaded empty catalog still says `0 stations`, and loading and Unavailable show their own texts. It raises its change whenever `HasNoMatches` or `CatalogStatusText` changes.
- **Select** (`SelectEntryCommand`): `Name` = entry name truncated to `NameMaxLength` and trimmed at the end; `Tag` = `entry.Tag` truncated to `TagMaxLength` (both truncations drop a high surrogate that the limit would split from its pair, so the text may end one character short, D83); `Url` = `entry.StreamUrl`; `SelectedEntry` = entry; `IsResultsOpen = false`; `HighlightedResult = null`; and **any pending search is cancelled** like a superseded one (`PendingSearch` completes, `Results` stay as they were), so a late result never reopens the overlay (D87, QA m2). These go through the normal setters, so a stale validation message clears.
- **Enter with a pending search** (D87, QA M1): a view-model method (named by the UI lane; this section records it when round 2 lands) runs the scheduled, not yet applied search at once (no remaining delay), applies it, and picks its first row as `SelectEntryCommand` does; with no match it picks nothing, the overlay stays closed and `StatusLineText` is `CatalogNoMatch`. Nothing is saved.
...
- **Search box keys** (handled in the view, calling the view model): Down → with the overlay closed and results present, opens it (the first row is then highlighted, §5.2) without moving; with it open, `MoveHighlight(+1)` (D85); Up → `MoveHighlight(-1)`; Enter → **while a search is pending** (`PendingSearch` not completed), run it at once and pick its first row, or nothing on no match, and mark handled, so a half-typed form is never saved (§5.2, D87); otherwise when `IsResultsOpen` and `HighlightedResult` is set, `SelectEntryCommand` and mark handled, otherwise not handled, so the default button saves (BHV-52/BHV-65), which after a no-match search saves or shows the name error (D87: the overlay is closed); since an open overlay with rows always has a highlight (D85 P2), typing then Enter picks the top match, and after a pick (overlay closed, nothing highlighted) Enter saves; PageUp / PageDown → scroll the detail pane by one page and mark handled (D85 P9). Keys with a modifier are not handled.
...
| D85 | test (`test-engineer`), after the UI lane | the D85 checks of §8 (CAT-10, 12, 13, 14) in `CatalogViewModelTests.cs` and `CatalogHeadlessTests.cs` (the UI lane's commits already updated the Phase 4b checks that pinned the old texts and keys) | product code |
...
| D87 | UI (`ui-engineer`), round 2 | `DialShift.App/ViewModels/StationEditorViewModel.cs` (the apply rule, `StatusLineText`, the empty-`Results` guard, a pick cancelling a pending search, the method that runs a pending search at once, a close demoting an in-flight search), `DialShift.App/Views/Dialogs/StationEditorDialog.axaml(.cs)` (the status line's binding, `noMatch` class and style, the overlay contents, the footer divider, the click rule, Enter with a pending search) | Services, Core, data, `UiText.CatalogNoMatch`'s text |
| D87 | test (`test-engineer`), after the UI lane | the D87 checks of §8 (CAT-12, 13, 14) in `CatalogViewModelTests.cs` and `CatalogHeadlessTests.cs`, replacing the checks that pin the no-match card | product code |
