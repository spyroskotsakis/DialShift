...
> `docs/decisions.md` (D59–D68 are the brief's §11 defaults, D69–D76 the ambiguities resolved in Phase 0, D77–D91
...
> §8 (CAT-01) and §9; implemented in `8e38215` (data, merged at `b3788dc`): 8,270 stations. The spec lane's D89 finding
> (a reopen did not set the open-when-applied flag) is fixed in `155027a` (UI lane, merged at `c50db51`; §5.2), and the
> test lane's D89 and D90 checks of §8 landed in `37c92b8` (merged at `0409c25`) and `56a9b20` (merged at `11114fb`),
> and the last two (CAT-12's Escape-then-Enter case, CAT-01's rule 9 mirror) in `f45aeeb` (merged at `8836ec3`). D91 (the result row's ` · ` separator gated on its brush contrast, its rendered pixels reported; after the first Windows CI run) amends §8 (CAT-14); implemented in `b2b81a9` (test lane, merged at `cceff38`). The CAT rows' final statuses (Phase 6, then the Windows CI run `36219966368`) are in `docs/acceptance-matrix.md` §11.
...
| CAT-16 | `CatalogPerf` (perf lane): load and search medians at the real count and at 10,000 synthetic entries, asserted against D69's budgets. **"Load" is D69's:** one `GetCatalogAsync` on a fresh `CatalogProvider` over the file (§4.2 steps 1–9, the time `catalog.loaded` reports), the median of 7 loads in one Release process after one warm-up load (before D102; since D102, after a warm-up to the JIT's steady state, below; the one-warm-up median is still printed, not gated). The **first load in a fresh process**, the only load the app performs, is printed next to it and not gated (D69). With the `Utf8JsonReader` parse (`80eed1f`, integration lane, Release, Apple M4 Max) it **meets the 50 ms budget on this machine**: median 38.5 ms over 10 fresh processes (37–41 ms), and 34 / 34 / 35 ms by `catalog.loaded` in the bundle smoke; D69's warm median measured 26.1–27.6 ms (§1). With the serializer at `d489465` it had measured 40–68 ms (integration lane) and 60–62 ms (spec lane). The cold load is reported, and measured again by the perf lane in Phase 5. **Measured at the JIT's steady state, retried only on a busy machine (D102; amends the measurement above, not the budgets):** after the cold first load and before anything is gated, 3 rounds of 20 untimed loads of each file, a search per query on each catalog and calibration passes, each round then a 250 ms pause; then the median of 7 loads of each file (each after a full collection) and per query one warm-up search and 7 samples. Attempts 1, 2 and 3 then agree whatever the suite order (the gated real load is about 13 ms on the M4 Max, where a standalone run measured 26 ms before). A calibration workload that runs none of the code under test (a `Utf8JsonReader` pass over the catalog bytes in memory, 10 untimed passes then the median of 5) is timed around each attempt: an attempt over budget whose slower calibration is within 1.2× of the fastest in the process fails at once; one over budget on a busy machine is measured again after 10 s, then 30 s, up to 3 attempts, each gate passing on its best; busy and over budget in all three fails as "runner contended …: inconclusive, re-run". Each attempt prints its calibrations, wall time and CPU time; each check line lists every attempt's value (`… (attempts: 131.0 ms busy, 12.7 ms; D102)`) | yes (Apple Silicon) | none required; Windows hardware is not measured (reported, not gated) |
...
Final gate (Phase 6, this Mac, 2026-09-26; orchestrator at `8836ec3`, under `caffeinate -i`): `dotnet build DialShift.slnx -c Release -warnaserror --no-incremental` → `Build succeeded. 0 Warning(s) 0 Error(s)`; `dotnet run --project DialShift.Tests -c Release` → `2702 passed, 6 skipped; 26/26 suites green.` (the 6 SKIPs Windows-only; `2694 passed` at `0409c25`, before the last checks); `data/.venv/bin/python data/build/app_catalog.py --self-test` → `app_catalog self-test: 320/320 checks passed`; the pipeline (data lane, `8e38215`): `working=8277 url_excluded=7 duplicates_removed=0 exported=8270`, `languages=42 unknown=0`. The CAT statuses this gave were 10 GREEN and 8 WINDOWS-PENDING (D75). **Windows CI (2026-09-26):** public CI `36219966368` at `cceff38` (after D91, `b2b81a9`) is green on both OSes: `windows-latest` `2627 passed, 25 skipped; 26/26 suites green.`, the native smoke 36/36 with the catalog check, both Windows package verifications with `station catalog with 8270 stations`, the `Install.ps1` cases and `All 32 fixtures got the expected verdict.`; `macos-latest` `2704 passed, 6 skipped; 26/26 suites green.`, the bundle smoke 34/34 and the 32 fixtures. `docs/acceptance-matrix.md` §11 now reads 15 GREEN and 3 NATIVE-PENDING. The test lane's final slice `f45aeeb` (merged at `8836ec3`: sleep-proof `TestDeadline` waits, CAT-12 Escape-then-Enter, CAT-01 rule 9 mirror) gave the same count in each of its 3 runs, one under load 38; two earlier failures were macOS Maintenance Sleep during the run, not load, so run unattended local suites under `caffeinate -i`.
