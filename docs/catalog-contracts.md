...
> `docs/decisions.md` (D59–D68 are the brief's §11 defaults, D69–D76 the ambiguities resolved in Phase 0, D77–D88
...
> (merged at `f7f9509`) adds the D85 checks of §8 and the D83/D86 regressions.
...
public sealed class StationEditorViewModel : EditorViewModel
{
    ...
    public bool PickOnEnter();                                   // D87: Enter in the search box; returns whether Enter
...
- **Enter** (`PickOnEnter()`, D87 as built in `65eb2f2`; the view's Enter in the search box sets `e.Handled` to its result). **With a search pending** (scheduled over the loaded catalog and not yet applied; while the catalog loads no search is scheduled, so this case does not arise then): the private `RunPendingSearchNow()` replaces the search in flight (a new generation, the old one cancelled) with one run synchronously on the UI thread on the current text and filters, with no remaining delay (a search is budgeted under 10 ms, D69), and applies it; its first row is then highlighted and picked as `SelectEntryCommand` does; with no match it picks nothing, the overlay stays closed and `StatusLineText` is `CatalogNoMatch`. It returns `true` either way, so nothing is saved (QA M1). A search that throws there is reported through `onError` as in the scheduled path. **With no search pending:** when `IsResultsOpen` and `HighlightedResult` is set, it picks the highlight and returns `true`; otherwise it returns `false`, and Enter goes on to the default button, Save (D85).
- **The "open when applied" flag** lives in the view model (a private field set by each schedule: `true` for a `SearchText` change, a filter change and Clear, and for the load completion only when text was typed during the load), not in the search request (as built, `65eb2f2`): any close other than a pick (`IsResultsOpen` set to `false` by Escape, focus into a form field or a press outside) clears it, so a search in flight applies its rows with the overlay left closed (D87 items 8, 9 (b)); a pick cancels the search instead (item 7).
...
As amended by D83 to match `c0bc4c5`, by D85 after the Phase 5 design review (implemented in `3a1afa2`, `2399d86` and `2ba28af`, merged at `e6606ec`), and by D87 (no match keeps the form visible; implemented in `cf70f69` and `65eb2f2`, merged at `9f0bc6c`).
