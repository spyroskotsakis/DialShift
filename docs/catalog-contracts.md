...
2. The path is a directory → `the path is a directory, not a file.`; it does not exist → `the file does not exist.`; it exists but is not a regular file (a FIFO, a character or block device, a socket) → `the path is not a regular file.`, checked **before the file is opened** (D81: opening a FIFO that has no writer blocks, so a check after opening never ran and the load never completed). The file is then opened read-only (`FileShare.Read`); `Length > MaxFileBytes` → `the file is <n> bytes, more than the 33554432 bytes allowed.`. An `IOException` or `UnauthorizedAccessException` at any step → `the file could not be read.`.
