...
| CAT-16 | `CatalogPerf` (perf lane): load and search medians at the real count and at 10,000 synthetic entries, asserted against D69's budgets. **"Load" is D69's:** one `GetCatalogAsync` on a fresh `CatalogProvider` over the file (§4.2 steps 1–9, the time `catalog.loaded` reports), the median of 7 loads in one Release process after one warm-up load. The **first load in a fresh process**, the only load the app performs, is printed next to it and not gated (D69). With the `Utf8JsonReader` parse (`80eed1f`, integration lane, Release, Apple M4 Max) it **meets the 50 ms budget on this machine**: median 38.5 ms over 10 fresh processes (37–41 ms), and 34 / 34 / 35 ms by `catalog.loaded` in the bundle smoke; D69's warm median measured 26.1–27.6 ms (§1). With the serializer at `d489465` it had measured 40–68 ms (integration lane) and 60–62 ms (spec lane). The cold load is reported, and measured again by the perf lane in Phase 5. **Each timing gate passes on its best of up to 3 attempts (D102):** the suite's measurement is attempt 1; while some gate has missed its budget in every attempt so far, the loads and searches are measured again, their lines labelled `…, attempt n`, never on code warmer than attempt 1's: in the same process in a full run (earlier suites have already brought the load path to the JIT's optimized code, and a repeat measured the same), in a fresh process of the test binary (`--catalog-perf-attempt <n>`) in a filtered run such as `--filter CatalogPerf` (a repeat in that process measured 20–50% faster); each check line lists every attempt's value (`… (attempts: 66.0, 17.1 ms; best of up to 3, D102)`). The budgets and the measurement are unchanged, and a regression over a budget fails all three attempts | yes (Apple Silicon) | none required; Windows hardware is not measured (reported, not gated) |
