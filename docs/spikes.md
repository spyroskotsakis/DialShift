...
**Windows CI coverage (HS-17 LV-01..LV-11, `DialShift.Tests/App/LibVlcEngineTests.cs`).** On `windows-latest`, `DialShift.Tests` runs the production `LibVlcPlaybackEngine` with `VideoLAN.LibVLC.Windows` 3.0.23.1 behind the real `PlaybackCoordinator`. It uses LibVLC's `adummy` output, because the runner has no audio device, and the in-process `LocalMediaServer`, which streams a synthesized WAV tone. This covers the local equivalents of T1 (refused), T2 (`.invalid`), T4 and T3 (403/500/404), T6 (redirect), T8 (user-info Basic auth; then a station on the same path without credentials; then a second path and realm without credentials → 401), T9 (HTML page), T11 (ICY), T12 (a stream that ends), T14 (never answers → watchdog `Stalled`, stepped clock), T15 (`ftp://`, at coordinator and engine level), and PLS/M3U playlist files, plus 8 × 20 rapid switches. **CI results:** in run `36102017139` every LV check passed except the 401 case, which then used the user-info station's path and played. In run `36103139541` LV-01..LV-11 passed except the same-path check in its first wording, which required a 401 before any credentials. LibVLC 3.0.23.1 sent the user-info station's stored credentials preemptively, with the first request to the same scheme://host:port and path. No request to any other path carried them: the second-realm 401 case and the protection-space check passed. The check now accepts preemptive reuse or `HttpError` and names the behavior the build showed; the `LibVlcPlaybackEngine` header and the README document it. In the same run, the `Pacific/Apia` whole-day-gap row (TZ-15, 2011-12-30) reports SKIP on Windows, because the runner's Windows time-zone data lacks Apia's 2011 date-line change. The "LibVLC (Windows)" column below stays pending for the public streams; they and audible output remain NC-03. Before the first CI run, the same checks LV-02..LV-11 ran 16 times through an out-of-repo harness against the Rosetta build above (†). After the fixes listed here, 6 consecutive runs passed all 61 checks in about 33 s each. The fixes were two test races and the ICY server behavior described next: a log line was read before the coordinator flushed it, and the test did not allow an `Opening` event that legitimately precedes a stop. No SIGILL occurred in any of these runs. **ICY finding:** LibVLC 3.0.4 requested a plain `HTTP/1.0 200` stream without `Icy-MetaData`, so no title can arrive. Only a Shoutcast v1 `ICY 200 OK` reply made LibVLC retry through its legacy HTTP module, which sent `Icy-MetaData: 1` and delivered the StreamTitle. The engine shows the station tag whenever no title arrives.
