...
| MX-10 | Windows LibVLC playback | playback | — | HS-17 (Windows: LV-01..LV-11, real LibVLC + coordinator, `adummy` output, local server; no audible check) | SMK + corpus | N/A | TODO |
...
| TZ-15 | Midnight / whole-day gap | Santiago/Beirut/Cuba-style 00:00→01:00 gap; `Pacific/Apia` 2011-12-30 | Fires on the correct shifted day (QA-N7) | test | Unit | TODO |
...
| NC-03 | Windows LibVLC real playback plus the media corpus (MP3, AAC, HLS, redirect, HTTPS failure, unreachable host, malformed URL, captive-portal-like). HS-17 LV-* covers the local transport cases, state mapping and failure kinds on `windows-latest` without audio; this check adds audible output, mute at volume 0, and the public MP3/AAC/HLS/HTTPS streams | Needs Windows audio and desktop | MX-10, MX-13 |
