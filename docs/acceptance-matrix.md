...
| CT-SCH-09..11 (D54) | Implemented, green locally (not yet in CI): `HH.mm` parses, fires and conflicts like `HH:mm`; every host culture's `HH:mm` text parses back; `FormatTime` is invariant under da-DK, fi-FI and a synthetic `.` culture | `DialShift.Tests/Core/SchedulerTests.cs` |
...
| CT-SET-12 (D54) | Implemented, green locally (not yet in CI) | `DialShift.Tests/Core/SettingsStoreTests.cs` |
