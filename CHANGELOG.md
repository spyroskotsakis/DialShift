...
- **Testing status of the setup:** CI is set up to build it on `windows-latest` and again on macOS, check that both builds are identical and that it holds exactly the zip's files, and run its install, upgrade, refusals and uninstall silently on `windows-latest`; the first such run is still pending. It has not yet been run by hand on a real Windows PC (native check NC-19): the wizard, SmartScreen and Smart App Control, and the Apps & features entry are unverified there.
